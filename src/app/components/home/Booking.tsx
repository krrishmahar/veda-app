...
const Booking:FC<EndpointProp> = ({ endpoint }) => {
    ...
    return (
        <>
                ...
                <div className="grid grid-cols-4 gap-11">
                    {currentItems.map((item) => (
                        <div key={item.id} className="bg-white shadow-2xl rounded-lg overflow-hidden relative mx-8 w-60">
                            <img
                                src={item.imageUrl || "/static/rooms/room-01.svg"}
                                alt={item.name}
                                className="w-full h-48 object-cover"
                                onError={(e: React.SyntheticEvent<HTMLImageElement>) => {
                                    e.currentTarget.src = "/static/rooms/room-01.svg";
                                }}
                            />
                            <button
                            ...
                            <div className="absolute right-2 rounded-md p-2 bg-white bg-opacity-70 z-10 bottom-[8.5rem]">
                                <p className="text-sm">
                                    <samp className="text-[#787878] font-extrabold">from </samp>
                                    <samp className="extra-lexa">{`${item.price} Rs.`}</samp>
                                </p>
                            </div>
                            ...
                            <div className="p-6 pt-2">
                                <div className="flex justify-between">
                                    {item.availability ? (
                                        <div className="flex items-center">
                                            <BsDoorOpenFill color="green" />
                                            <samp className="extra-lexa text-green-600"> Available</samp>
                                        </div>
                                    ) : (
                                        <div className="flex items-center">
                                            <BsDoorOpenFill color="red" />
                                            <samp className="extra-lexa text-red-600"> Unavailable</samp>
                                        </div>
                                    )}
                                    {endpoint === "/api/rooms" ? (
                                        <div className="flex items-center">
                                ...
                                {endpoint === "/api/halls" && (
                                    <div className="icons flex justify-evenly my-1">
                                        {item.food && <GiKnifeFork size={16} />}
                                        {item.ac && <TbAirConditioning size={16} />}
                                    </div>
                                )}
                                <button
                                    className="bg-white text-black py-2 px-4 border font-extrabold rounded-md mt-2 w-full"
            ...
            <div className="flex justify-end px-6">
                <Link href="/roompage">
                    <button className="m-4 border border-black rounded-full px-6 py-[0.35rem] font-semibold">
                        View more...
                    </button>
                </Link>
            </div>
                ...
                {Array.from({ length: totalPages }, (_, index) => (
                    <button
                        ...
                        className={`mx-1 w-3 h-3 rounded-full ${
                            currentPage === index + 1 ? 'bg-gray-900' : 'bg-gray-300'
                        }`}
                    />
                ))}
            </div>
